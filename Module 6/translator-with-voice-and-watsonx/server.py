...
import os
# Allows the server.py file to access these imported functions from the worker.py file
...
def speech_to_text_route():
    ...
    text = speech_to_text(audio_binary) # Call speech_to_text (defined in worker.py) function to transcribe the speech
    # Return the response to user in JSON format(as expected by the frontend)
    response = app.response_class(
        # Actual data we want to send in the body of the HTTP response
...
def process_message_route():
    ...
    # Call watsonx_process_message(from worker.py) function to process the user's message and get a response back
    ...
    # (since json format can only store textual data)

# The server is how the application will run and communicate with all of the services

import logging
import os
from flask import Flask, render_template, request, jsonify
from flask_cors import CORS
import worker  # Import the worker.py file

# Initialize Flask app and CORS
# When a user interacts with the chatbot, the request will be sent to the Flask backend,
# Flask will then process the request and send it back to the appropriate service
app = Flask(__name__)
...
# Define the route for processing messages(prompt about the processed document)
...
# Define the route for processing documents(when a user uploads a PDF)
...
# Run the Flask app
# (if __name__ == "__main__" checks if the script is being run directly and not just being imported)
